...
static void _dump_partition(void)
{
    ...
    printf("\x1b[32m======= PtTable_Config @%p=======\r\n\x1b[00m", part);
    ...
    printf("\x1b[32m crc32 0x%08X\r\n\x1b[00m", (unsigned int)part->ptTable.crc32);

    printf("\x1b[32midx  type device activeIndex     name   Address[0]  Address[1]  Length[0]   Length[1]   age\x1b[00m\r\n");
...
void hal_update_mfg_ptable(void)
{
    ...
    blog_info("====================\r\n");
    if (0 == hal_boot2_get_active_entries_byname((uint8_t *)PT_OTA_TYPE_NAME, (HALPartition_Entry_Config *)(&ptEntry_fw)))
    { // ota
        if (0 == hal_boot2_get_active_entries_byname((uint8_t *)PT_MEDIA_TYPE_NAME, (HALPartition_Entry_Config *)(&ptEntry_media)))
        { // media
